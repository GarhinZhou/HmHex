...
// OHOS toolchain workaround: the bundled clang does not implement P0960
// (aggregate initialization with parentheses) and its list-initialization
// narrowing check is buggy (rejects valid signed->unsigned widening), so a
// plain paren/brace initialization cannot cover all cases. Dispatch on
// aggregate-ness instead: aggregates use braces (C++17 style aggregate init),
// everything else uses parentheses (regular constructor call).
#if _LIBCPP_STD_VER > 17

template <class _Tp, class... _Args>
_LIBCPP_HIDE_FROM_ABI constexpr _Tp* __construct_at_impl(_Tp* __location, _Args&&... __args) {
  if constexpr (is_aggregate_v<_Tp>)
...
_LIBCPP_HIDE_FROM_ABI constexpr _Tp* construct_at(_Tp* __location, _Args&&... __args) {
  ...
  return __construct_at_impl(__location, _VSTD::forward<_Args>(__args)...);
}
