...
namespace pl {
    ...
    private:
        ...
        struct Function {
            ...
            // Workaround: the OHOS libc++ (libcxx-ohos) construct_at does not support
            // C++20 aggregate initialization, so provide a regular constructor.
            Function(const api::Namespace &ns, std::string name_, api::FunctionParameterCount count,
                     api::FunctionCallback cb, bool isDangerous)
                : nameSpace(ns), name(std::move(name_)), parameterCount(count),
                  callback(std::move(cb)), dangerous(isDangerous) { }
        };
