...
namespace romfs {
    ...
    // NOTE: These wrappers must be `static` (internal linkage). In the desktop
    // build every plugin is a separate .so and the `inline` versions each get
    // their own copy, but when plugins are statically linked into a single
    // binary the multiple inline definitions collide (ODR) and the linker
    // keeps only one of them, so every call would resolve to the same romfs
    // project. `static` gives every translation unit its own copy, which is
    // expanded with that TU's LIBROMFS_PROJECT_NAME.
    [[nodiscard]] ROMFS_VISIBILITY inline static const Resource& get(const std::filesystem::path &path) { return impl::ROMFS_CONCAT(get_, LIBROMFS_PROJECT_NAME)(path); }
    [[nodiscard]] ROMFS_VISIBILITY inline static std::vector<std::filesystem::path> list(const std::filesystem::path &path = {}) { return impl::ROMFS_CONCAT(list_, LIBROMFS_PROJECT_NAME)(path); }
    [[nodiscard]] ROMFS_VISIBILITY inline static std::string_view name() { return impl::ROMFS_CONCAT(name_, LIBROMFS_PROJECT_NAME)(); }
