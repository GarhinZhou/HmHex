...
namespace hex {
    ...
    protected:
        ...
        /**
         * @brief Whether the window should be centered every time it is opened
         *        (OHOS: floating tool windows overlap the menu bar at the
         *        default top-left cascade position, making their title bar
         *        undraggable). Off by default; enabled for View::Floating.
         */
        void setCenterOnOpen(bool enabled) { m_centerOnOpen = enabled; }
        [[nodiscard]] bool getCenterOnOpen() const { return m_centerOnOpen; }
    ...
    public:
        explicit Floating(UnlocalizedString unlocalizedName, const char *icon) : Window(std::move(unlocalizedName), icon) {
            // Floating tool windows must not overlap the menu bar; center
            // them every time they are opened (like the About modal).
            this->setCenterOnOpen(true);
        }
