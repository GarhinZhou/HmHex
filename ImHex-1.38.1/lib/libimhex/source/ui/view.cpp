    ...
    void View::Window::draw(ImGuiWindowFlags extraFlags) {
        if (this->shouldDraw()) {
            ...
            ImGui::SetNextWindowSizeConstraints(this->getMinSize(), this->getMaxSize());
                ...
                // The default top-left cascade position overlaps the menu bar,
                // which makes the window title bar undraggable (only the title
                // bar can move a window). Center windows on open instead.
                // Floating windows (store, theme manager, log console, ...)
                // are centered every time; regular windows only on first use
                // so dragged positions are remembered. NOTE: never use
                // ImGuiCond_Appearing for regular windows - it also fires for
                // docked windows and tears them out of the tiled layout.
                if (this->getCenterOnOpen())
            ...
            if (ImGui::Begin(title.c_str(), &this->getWindowOpenState(), ImGuiWindowFlags_NoCollapse | extraFlags | this->getWindowFlags())) {
                #if defined(IMHEX_OHOS_PORT)
                    // Independent (undocked) windows that would be unusable
                    // get repositioned: those whose title bar ended up outside
                    // the main viewport (stale imgui.ini coordinates) are
                    // pulled back to the center, and those whose title bar
                    // overlaps the menu bar are pushed below it - the menu
                    // bar eats clicks, making such windows undraggable.
                    // Docked windows are skipped, so the tiled layout is
                    // never disturbed. Windows currently being dragged are
                    // left alone.
                    if (!ImGui::IsWindowDocked() && !ImGui::GetIO().MouseDown[ImGuiMouseButton_Left]) {
                        const auto windowPos    = ImGui::GetWindowPos();
                        const auto windowSize   = ImGui::GetWindowSize();
                        const auto viewport     = ImGui::GetMainViewport();
                        const auto titleBarSize = ImGui::GetFrameHeight();

                        const bool outsideLeft = windowPos.x + titleBarSize < viewport->WorkPos.x;
                        const bool outsideTop  = windowPos.y + titleBarSize < viewport->WorkPos.y;
                        const bool underMenuBar = windowPos.y < viewport->WorkPos.y + ImGui::GetTextLineHeightWithSpacing();

                        if (outsideLeft || outsideTop || underMenuBar) {
                            auto center = viewport->GetCenter();
                            // Keep the title bar below the menu bar so the
                            // window stays draggable.
                            const auto minCenterY = viewport->WorkPos.y + ImGui::GetTextLineHeightWithSpacing() + 4.0F + windowSize.y / 2.0F;
                            if (center.y < minCenterY)
                                center.y = minCenterY;
                            ImGui::SetWindowPos(center - windowSize / 2.0F, ImGuiCond_Always);
                        }
                    }
                #endif
