...
namespace hex {
    ...
    // Implemented in the OHOS entry layer (xcomponent_entry.cpp): opens the
    ...
    void openWebpage(std::string url) {
        if (!url.contains("://"))
            ...
            // Open the URL through the ArkTS layer (system browser / app picker).
            ohosNotifyOpenWebpage(url.c_str());
        #elif defined(OS_LINUX)
    ...
    void* getContainingModule(void* symbol) {
        ...
        return nullptr;
#elif !defined(OS_WEB) && !defined(__OHOS__)
        Dl_info info = {};
