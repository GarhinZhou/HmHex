...
namespace hex::paths {
    ...
    static std::vector<std::fs::path> getPluginPaths() {
        ...
        // The OHOS app sandbox linker namespace only permits dlopen from the
        // HAP's native library directory; the plugins are packaged there by
        // the build (named "<plugin>.hexplug.so").
        if (const char *pluginDir = std::getenv("IMHEX_OHOS_PLUGIN_DIR")) {
            return { pluginDir };
        }
        #endif
    ...
    namespace impl {
        ...
        std::vector<std::fs::path> PluginPath::all() const {
            #if defined(IMHEX_OHOS_PORT)
            // On OHOS the plugin directory (IMHEX_OHOS_PLUGIN_DIR) already is
            // the final plugin folder (the HAP native library directory); do
            // not append the "plugins" postfix.
