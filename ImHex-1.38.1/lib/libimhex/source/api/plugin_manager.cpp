...
namespace hex {
    ...
    static uintptr_t loadLibrary(const std::fs::path &path) {
            ...
            if (handle == 0) {
                ...
                log::error("Loading library '{}' failed: {}!", wolv::util::toUTF8String(path.filename()), dlError != nullptr ? dlError : "(null)");
                OH_LOG_Print(LOG_APP, LOG_ERROR, 0xD002D00, "ImHexNative",
                             "plugin dlopen failed: %{public}s", dlError != nullptr ? dlError : "(null)");
                return 0;
            }
    ...
    bool PluginManager::load(const std::fs::path &pluginFolder) {
        ...
        for (auto &pluginPath : std::fs::directory_iterator(pluginFolder)) {
            ...
            // On OHOS the plugins are packaged as "<name>.hexpluglib.so" /
            // "<name>.hexplug.so" inside the HAP native library directory so
            // the sandbox linker namespace allows dlopen of them.
            bool isLibraryPlugin = pluginPath.is_regular_file() &&
                                   pluginPath.path().filename().string().find(".hexpluglib.so") != std::string::npos;
            #else
            bool isLibraryPlugin = pluginPath.is_regular_file() && pluginPath.path().extension() == ".hexpluglib";
            #endif
            if (isLibraryPlugin) {
        ...
        for (auto &pluginPath : std::fs::directory_iterator(pluginFolder)) {
            #if defined(IMHEX_OHOS_PORT)
            bool isPlugin = pluginPath.is_regular_file() &&
                            pluginPath.path().filename().string().find(".hexplug.so") != std::string::npos &&
                            pluginPath.path().filename().string().find(".hexpluglib.so") == std::string::npos;
            #else
            bool isPlugin = pluginPath.is_regular_file() && pluginPath.path().extension() == ".hexplug";
            #endif
            if (isPlugin) {
                if (!isPluginLoaded(pluginPath.path())) {
                    getPluginsMutable().emplace_back(pluginPath.path());
