...
namespace hex {
    ...
    namespace ImHexApi::System {
        ...
        // Default to no frame rate limit on the OHOS port (values > 200
        // disable the limit in the main loop).
        static float s_targetFPS = 201.0F;
        ...
        bool updateImHex(UpdateType updateType) {
                ...
                return true;
            #elif defined(IMHEX_OHOS_PORT)
                // Updates are delivered through the app store on OHOS; the
                // imhex-updater executable does not exist, and iterating the
                // bundle directory would throw a filesystem_error that unwinds
                // through the ImGui frame and corrupts its window/disabled
                // stacks (crash at EndFrame). Just report "no updater".
                std::ignore = updateType;
                return false;
            #else
