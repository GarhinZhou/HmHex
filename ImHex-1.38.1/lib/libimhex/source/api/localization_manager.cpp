...
namespace hex {
    ...
    // Protects s_unlocalizedNames: plugins register strings from the init-task
    // thread while the render thread and the OHOS UI thread construct Lang
    // objects and resolve them concurrently. Without any lock the map gets
    // corrupted under contention (crash in Lang::Lang, SIGSEGV).
    //
    // A plain std::mutex made the UI stutter: Lang::get()/Lang construction
    // run every frame on the render thread, and serializing even the
    // read-only paths across threads caused cache-line contention. A shared
    // mutex keeps concurrent readers (the render path) parallel and only
    // serializes writers (startup registration) against readers.
    static std::shared_mutex s_unlocalizedNamesMutex;

    Lang::Lang(std::string_view unlocalizedString) : m_entryHash(LangConst::hash(unlocalizedString)) {
        std::unique_lock lock(s_unlocalizedNamesMutex);
    ...
    const char *Lang::get() const {
        ...
        if (it == lang.end()) {
            std::shared_lock lock(s_unlocalizedNamesMutex);
