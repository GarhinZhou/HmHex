...
namespace hex::ui {
    ...
    static CustomEncodingData queryCustomEncodingData(prv::Provider *provider, const EncodingFile &encodingFile, u64 address) {
        ...
        auto encodingResult = encodingFile.getEncodingFor(buffer);
        const auto &decoded = encodingResult.first;
        const auto &advance = encodingResult.second;
                            ...
                            if (size_t(y) < m_encodingLineStartAddresses.size() || singleByteEncoding) {
                                ...
                                if (singleByteEncoding) {
                                    ...
                                    if (m_encodingLineStartAddresses[y] >= bytesPerRow) {
                                        encodingData.emplace_back(y * bytesPerRow + m_provider->getBaseAddress() + m_provider->getCurrentPageAddress(), CustomEncodingData{std::string("."), 1, ImGuiExt::GetCustomColorU32(ImGuiCustomCol_AdvancedEncodingUnknown)});
                                        m_encodingLineStartAddresses.push_back(0);
                                    } else {
