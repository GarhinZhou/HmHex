    ...
    void addToolbarItems() {
        ...
        ContentRegistry::UserInterface::addToolbarItem([] {
            ...
            for (const auto &menuItem : ContentRegistry::UserInterface::impl::getToolbarMenuItems()) {
                ...
                // The toolbar buttons derive their ImGui ID from the icon
                // glyph alone, so two toolbar items that share an icon collide.
                // Push the menu item's identity so every button gets a unique ID.
                ImGui::PushID(menuItem->unlocalizedNames.back().get().c_str());
