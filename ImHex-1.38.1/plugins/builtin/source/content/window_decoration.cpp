...
namespace hex::plugin::builtin {
    ...
    namespace {
        ...
        void drawTitleBar() {
            ...
            if (!titleBarButtons.empty()) {
                    ...
                    // OHOS: the top-right corner belongs to the system caption
                    // buttons (immersive mode); place our buttons right next to
                    // the centered search box instead.
                    ImGui::SetCursorPosX(searchBoxPos.x + searchBoxSize.x + 10_scaled);
                #else
        ...
        void drawMainMenu([[maybe_unused]] float menuBarHeight) {
            #if defined(IMHEX_OHOS_PORT)
                // Taller menu bar (File/Edit/View...) for easier clicking on
                // 2in1 touch screens. Pushed first so the existing
                // PopStyleVar(2) below still pops WindowBorderSize and
                // FrameBorderSize.
    ...
    void addWindowDecoration() {
        EventFrameBegin::subscribe([]{
            ...
            #if defined(IMHEX_OHOS_PORT)
                // The OHOS menu bar uses a taller FramePadding; it must be
                // active while Begin() computes MenuBarHeight, otherwise the
                // dock area is sized too large and its bottom is clipped.
                ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(ImGui::GetStyle().FramePadding.x, 8_scaled));
            #endif
