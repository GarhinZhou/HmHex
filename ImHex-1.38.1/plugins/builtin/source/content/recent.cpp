            ...
            for (const auto &entry : std::fs::directory_iterator(backupPath)) {
                if (entry.is_regular_file() && entry.path().extension() == ".hexproj") {
                    ...
                    try {
                        ...
                        // The localized format string may contain invalid chrono specifiers (e.g. translated %Y to %年).
                        // Fall back to a well-known valid format so the auto backup list stays usable.
                        result.emplace(
                            fmt::format("Backup from {:%Y-%m-%d %H:%M:%S}", *localTm),
                            entry.path(),
                            *localTm
                        );
                    }
