...
namespace hex::plugin::builtin {
    ...
    namespace {
        ...
        // Implemented in the OHOS entry layer (xcomponent_entry.cpp); forwards
        // the "always on top" setting toggle to the ArkTS system window API.
        ...
        */
        ...
        // "Always on top" toggle: forwards the state change to the ArkTS
        // layer, which applies it via the system window API. The value is
        // persisted in the settings file and re-applied on startup.
        class AlwaysOnTopWidget : public ContentRegistry::Settings::Widgets::Widget {
        public:
            bool draw(const std::string &name) override {
                bool enabled = m_value;

                if (ImGui::Checkbox(name.data(), &enabled)) {
                    m_value = enabled;

                    ohosNotifySetWindowTopmost(enabled);
                    return true;
                }
        ...
        class FPSWidget : public ContentRegistry::Settings::Widgets::Widget {        public:
            bool draw(const std::string &name) override {
        ...
        private:
            // Default to "Unlimited" (> 200) on the OHOS port.
            int m_value = 201;
        };
            ...
            bool draw(const std::string &) override {
                ...
                if (ImGui::BeginTable("##top_level", 2, ImGuiTableFlags_None, ImGui::GetContentRegionAvail())) {
                    ...
                    if (ImGuiExt::BeginSubWindow("hex.builtin.setting.toolbar.icons"_lang, nullptr, ImGui::GetContentRegionAvail())) {
                        ...
                        const auto columnCount = std::max<int>(1, int(ImGui::GetContentRegionAvail().x / itemWidth));
                        // Leave room at the bottom for the drop zone so the
                        // icon table keeps its full size.
                        const auto tableSize = ImGui::GetContentRegionAvail() - ImVec2(0, 40_scaled);
                        if (ImGui::BeginTable("##icons", columnCount, ImGuiTableFlags_SizingStretchSame, tableSize)) {
                            ImGui::TableNextRow();
                        ...
                        // Drop zone at the bottom for adding new items from
                        // the left list. Fixed height so it keeps real
                        // geometry even when the toolbar is completely empty;
                        // drawn with a visible border, fill and hint text.
                        const auto dropZoneMin = ImGui::GetCursorScreenPos();
                        const auto dropZoneSize = ImVec2(ImGui::GetContentRegionAvail().x, 32_scaled);
                        ImGui::InvisibleButton("##toolbar_drop_zone", dropZoneSize);
                        const auto dropZoneMax = dropZoneMin + dropZoneSize;

                        {
                            const auto drawList = ImGui::GetWindowDrawList();
                            const bool hovered = ImGui::IsItemHovered();
                            drawList->AddRectFilled(dropZoneMin, dropZoneMax,
                                ImGui::GetColorU32(ImGuiCol_Button, hovered ? 0.30F : 0.15F), 4.0F);
                            drawList->AddRect(dropZoneMin, dropZoneMax,
                                ImGui::GetColorU32(ImGuiCol_Border), 4.0F, 0, 1.0F);

                            // Localized hint text, truncated with an ellipsis
                            // if it does not fit the drop zone width.
                            auto text = std::string(Lang("hex.builtin.setting.toolbar.drop_hint"));
                            while (ImGui::CalcTextSize(text.c_str()).x > dropZoneSize.x - 12_scaled && text.size() > 1)
                                text.pop_back();

                            const auto textSize = ImGui::CalcTextSize(text.c_str());
                            drawList->AddText(dropZoneMin + (dropZoneSize - textSize) * 0.5F,
                        ...
                        if (ImGui::BeginDragDropTarget()) {
                            if (auto payload = ImGui::AcceptDragDropPayload("MENU_ITEM_PAYLOAD"); payload != nullptr) {
                                ...
                                // Assign the next free index after the current
                                // maximum. Using the item count (currIndex)
                                // collides when indices are not contiguous
                                // (e.g. after removals), and the settings are
                                // stored in a std::map keyed by index — a
                                // duplicate index silently drops one item.
                                i32 nextIndex = 0;
                                for (const auto &[priority, other] : ContentRegistry::UserInterface::impl::getMenuItems()) {
                                    if (other.toolbarIndex >= nextIndex)
                                        nextIndex = other.toolbarIndex + 1;
                                }
                                menuItem->toolbarIndex = nextIndex;
                                changed = true;
                            }
    ...
    void registerSettings() {
        ...
        /* General */
        {
            ...
            ContentRegistry::Settings::add<Widgets::Checkbox>("hex.builtin.setting.general", "", "hex.builtin.setting.general.show_tips", false);
                ...
                ContentRegistry::Settings::add<AlwaysOnTopWidget>("hex.builtin.setting.general", "", "hex.builtin.setting.general.always_on_top");
                // Re-apply the persisted always-on-top state after startup.
                EventManager::subscribe<EventImHexStartupFinished>([] {
                    const bool topmost = ContentRegistry::Settings::read<bool>("hex.builtin.setting.general", "hex.builtin.setting.general.always_on_top", false);
                    ohosNotifySetWindowTopmost(topmost);
                });
            #endif
        ...
        /* Interface */
        {
...

            {
                ...
                ContentRegistry::Settings::add<Widgets::DropDown>("hex.builtin.setting.interface", "hex.builtin.setting.interface.language", "hex.builtin.setting.interface.language", languageNames, languageCodes, "zh-CN");
            }
        ...
        /* Experiments */
        {
            ...
            EventImHexStartupFinished::subscribe([]{
                for (const auto &experimentEntry : ContentRegistry::Experiments::impl::getExperiments()) {
                    const auto &name = experimentEntry.first;
                    const auto &experiment = experimentEntry.second;
