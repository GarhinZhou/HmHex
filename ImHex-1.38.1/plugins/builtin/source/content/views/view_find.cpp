...
namespace hex::plugin::builtin {
    ...
    std::vector<hex::ContentRegistry::DataFormatter::impl::FindOccurrence> ViewFind::searchValue(Task &task, prv::Provider *provider, Region searchRegion, const SearchSettings::Value &settings) {
        ...
        auto minResult = parseNumericValueInput(inputMin, settings.type);
        auto maxResult = parseNumericValueInput(inputMax, settings.type);
        const auto &validMin = std::get<0>(minResult);
        const auto &min = std::get<1>(minResult);
        const auto &sizeMin = std::get<2>(minResult);
        const auto &validMax = std::get<0>(maxResult);
        const auto &max = std::get<1>(maxResult);
        const auto &sizeMax = std::get<2>(maxResult);
