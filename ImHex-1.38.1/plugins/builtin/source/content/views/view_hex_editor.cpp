...
namespace hex::plugin::builtin {
    ...
    void ViewHexEditor::drawHelpText() {
        ImGuiExt::TextFormattedWrapped("这是你日常使用的主要视图。它显示当前打开的数据源的原始数据，以及由 HmHex 其他部分生成的各种高亮。");
        ImGui::NewLine();
        ImGuiExt::TextFormattedWrapped("你可以通过双击单元格来编辑它。默认情况下，这会覆盖该单元格的原有值。如果要改为插入新字节，请开启 {} 选项（位于 {} 菜单中）。",
            "hex.builtin.view.hex_editor.menu.edit.insert_mode"_lang, "hex.builtin.menu.edit"_lang
        );
        ImGui::NewLine();
        ImGuiExt::TextFormattedWrapped("在这个视图中右键点击任意位置，或使用底部工具栏中的开关和选项，可以找到更多功能，包括更改每行显示的字节数、文本显示编码、迷你地图和单元格的数据格式。",
            "hex.builtin.view.hex_editor.menu.edit.insert_mode"_lang, "hex.builtin.menu.edit"_lang
        );
    }
