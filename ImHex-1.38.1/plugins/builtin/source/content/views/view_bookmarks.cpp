    ...
    void ViewBookmarks::drawContent() {
        ...
        if (open) {
            ...
            for (auto it = m_bookmarks->begin(); it != m_bookmarks->end(); ++it) {
                auto &bookmarkEntry = *it;
                auto &entry = bookmarkEntry.entry;
                auto &highlightVisible = bookmarkEntry.highlightVisible;
                auto &commentDisplay = bookmarkEntry.commentDisplay;
                auto &region = entry.region;
                auto &name = entry.name;
                auto &comment = entry.comment;
                auto &color = entry.color;
                auto &locked = entry.locked;
                auto &bookmarkId = entry.id;
