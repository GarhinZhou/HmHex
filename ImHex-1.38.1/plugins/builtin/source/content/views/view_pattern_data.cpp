...
namespace hex::plugin::builtin {
    ...
    void ViewPatternData::drawHelpText() {
        ImGuiExt::TextFormattedWrapped("该视图显示之前执行的模式脚本生成的模式树。只有模式成功执行后才会激活。");
        ImGui::NewLine();
        ImGuiExt::TextFormattedWrapped("点击树中的模式将选中十六进制编辑器视图中对应的字节，并跳转到模式编辑器视图中的定义位置。此外，你可以双击模式来编辑其值，编辑后会自动更新已加载数据源中的字节。");
    }
