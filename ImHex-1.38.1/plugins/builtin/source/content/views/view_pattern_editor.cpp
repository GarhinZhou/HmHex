...
namespace hex::plugin::builtin {
    ...
    void ViewPatternEditor::evaluatePattern(const std::string &code, prv::Provider *provider) {
        ...
        auto &runtimeLock = ContentRegistry::PatternLanguage::getRuntimeLock();
        if (!runtimeLock.try_lock()) {
            // An evaluation is currently running and holds the runtime lock
            // for its whole duration. Blocking here would freeze the render
            // thread (and with it the whole UI) until that evaluation
            // finishes — which can take many seconds for large files. Skip
            // this request instead; the running evaluation already reflects
            // the current code (auto re-evaluations are triggered for
            // changes made while it runs).
            return;
        }
        auto lock = std::unique_lock(runtimeLock, std::adopt_lock);
        ...
        TaskManager::createTask("hex.builtin.view.pattern_editor.evaluating", TaskManager::NoProgress, [this, code, provider](auto &task) {
            ...
            #if defined(OS_LINUX)
                // Lower this thread's nice value so a long-running evaluation
                // does not starve the render thread: with the pattern editor
                // and the hex editor open side by side the evaluation would
                // otherwise make both views stutter for the whole evaluation.
                // (musl setpriority accepts a thread id as the pid argument.)
                if (setpriority(PRIO_PROCESS, static_cast<id_t>(syscall(SYS_gettid)), 10) != 0) {
                    hex::log::error("Failed to lower evaluation thread priority: {}", strerror(errno));
                } else {
                    hex::log::info("Evaluation thread priority lowered (nice +10)");
                }
            #endif
        ...
        ContentRegistry::UserInterface::addMenuItemSubMenu({ "hex.builtin.menu.edit", "hex.builtin.view.pattern_editor.menu.edit.place_pattern" }, ICON_VS_LIBRARY, 3000,
            [&, this] {
                ...
                if (menu::beginMenu("hex.builtin.view.pattern_editor.menu.edit.place_pattern.custom"_lang, hasPlaceableTypes)) {
                    ...
                    for (const auto &typeEntry : types) {
                        const auto &typeName = typeEntry.first;
                        const auto &type = typeEntry.second;
    ...
    void ViewPatternEditor::openPatternFile(bool trackFile) {
        ...
        ui::PopupNamedFileChooser::open(
            ...
            [this](const std::fs::path &path, const std::fs::path &adjustedPath) -> std::string {
                ...
                // Fast scan for a "#pragma description <text>" line. The
                // bundled patterns all use the C-style pragma, which the full
                // pl lexer (getPragmaValues) does not even match (it expects
                // @pragma("key", "value")), so running it over every bundled
                // pattern file only wasted ~10s on the first open.
                std::string description;
                wolv::io::File file(path, wolv::io::File::Mode::Read);
                const auto content = file.readString();
                constexpr static std::string_view DescriptionPragma = "#pragma description";
                if (const auto pos = content.find(DescriptionPragma); pos != std::string::npos) {
                    const auto valueStart = pos + DescriptionPragma.size();
                    const auto lineEnd = content.find_first_of("\r\n", valueStart);
                    description = content.substr(valueStart, lineEnd == std::string::npos ? std::string::npos : lineEnd - valueStart);
                    description = wolv::util::trim(description);
                }

                if (!description.empty()) {
                    m_patternNames[path] = fmt::format("{} ({})", description, fileName);
                } else {
