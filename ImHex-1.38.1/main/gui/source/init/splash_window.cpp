...
namespace hex::init {
    ...
    void WindowSplash::fullFrame() {
        ...
        // On OHOS the splash renders into the full-screen XComponent surface
        // while its drawing code uses the fixed 640x400 splash layout; center
        // the layout in the actual surface.
        #if defined(IMHEX_OHOS_PORT)
            const auto displaySize = ImGui::GetIO().DisplaySize;
            const ImVec2 offset((displaySize.x - WindowSize.x) / 2.0F, (displaySize.y - WindowSize.y) / 2.0F);
        #else
            const ImVec2 offset(0, 0);
        #endif
        ...
        auto drawList = ImGui::GetBackgroundDrawList();
        {
            ...
            // Draw the splash screen background
            drawList->AddImage(m_splashBackgroundTexture, offset, offset + WindowSize);

            {
                ...
                const auto highlightBytes = [&](ImVec2 start, size_t count, ImColor color, float opacity) {
                    ...
                    const auto hexStart = ImVec2(27, 127) + offset;
            ...
            drawList->AddImage(m_splashTextTexture, offset, offset + WindowSize);

            // Draw the "copyright" notice
            drawList->AddText(offset + ImVec2(35, 85), ImColor(0xFF, 0xFF, 0xFF, 0xFF), fmt::format("WerWolv\n2020 - {0}", &__DATE__[7]).c_str());
            ...
            drawList->AddText(offset + ImVec2((WindowSize.x - ImGui::CalcTextSize(VersionInfo.c_str()).x) / 2, 105), ImColor(0xFF, 0xFF, 0xFF, 0xFF), VersionInfo.c_str());
        }
        ...
        // Draw the task progress bar
        {
            ...
            const auto progressBackgroundStart = offset + ImVec2(99, 357);
