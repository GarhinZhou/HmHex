...
namespace hex {
    ...
    void Window::fullFrame() {
        ...
        try {
            ...
            try {
                ...
                // Recovery itself failed (a nested error-recovery step threw
                // again). Force ImGui back into a clean frame state so the
                // next frame renders normally instead of leaving the window
                // permanently black (WithinFrameScope stuck / EndFrame never
                // completed).
                log::error("ImGui error recovery failed, resetting frame state");
                if (auto g = ImGui::GetCurrentContext(); g != nullptr) {
    ...
    void Window::initGLFW() {
        ...
        // Set up initial window size
        {
            ...
            // On OHOS the window is the full-screen XComponent surface: the
            // "monitor" is a fixed 1920x1080 placeholder, so clamping would
            // shrink the reported main window size and mis-center popups.
    ...
    void Window::initImGui() {
        ...
        // Recoverable ImGui errors (e.g. a Begin/End mismatch detected when an
        // exception unwound the frame mid-way) must not abort the app.
        // ErrorRecoveryTryToRecoverState() is designed to log the problem,
        // repair the window stack and continue with the next frame. ImHex's
        // IM_ASSERT override routes to hex::dbg::assertionHandler() which
        // aborts, so disable the assert part of the recoverable-error path.
        io.ConfigErrorRecoveryEnableAssert = false;
        // Forward recoverable-error messages (e.g. "Missing EndChild()") to
        // the log so the underlying problem can be diagnosed.
