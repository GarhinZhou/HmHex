...
namespace hex {
    ...
    bool isFileInPath(const std::fs::path &filename) {
        ...
        for (auto dirRange : std::views::lazy_split(optPathVar.value(), ':')) {
            std::string dir;
            for (char c : dirRange)
                dir += c;
            if (std::fs::exists(std::fs::path(dir) / filename)) {
                return true;
            }
        }
        return false;
    ...
    void enumerateFonts() {
        ...
        // Use the error_code overloads everywhere: on OHOS the sandbox has no
        // permission to stat /system/fonts etc., and throwing filesystem_error
        // from the render thread would kill the whole UI.
        for (const auto &directory : FontDirectories) {
            std::error_code ec;
            if (!std::fs::exists(directory, ec) || ec)
                continue;

            std::fs::recursive_directory_iterator it(
                directory, std::fs::directory_options::skip_permission_denied, ec);
            if (ec)
                continue;
            std::fs::recursive_directory_iterator end;
